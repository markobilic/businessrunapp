...
new class extends Component {
    ...
    public function mount($runnerId)
    {
        if($runnerId)
        {
            ...
            $runner = Runner::findOrFail($runnerId);
            ...
            if($runner)
            {
                ...
                if(!auth()->user()->hasRole(['superadmin', 'organizer']))
                {
                    if($this->selectedRunner->captain->user->id != auth()->user()->id)
                    {
                        return redirect()->route('runners.list');
                    }
                }

                $this->captain_id = $this->selectedRunner->captain_id;
                $this->name = $this->selectedRunner->name;
                $this->last_name = $this->selectedRunner->last_name;
                $this->email = $this->selectedRunner->email;
                $this->phone = $this->selectedRunner->phone;
                $this->sex = $this->selectedRunner->sex;
                $this->date_of_birth = $this->selectedRunner->date_of_birth;
                $this->work_position_id = $this->selectedRunner->work_position_id;
                $this->work_sector_id = $this->selectedRunner->work_sector_id;
                $this->week_running_id = $this->selectedRunner->week_running_id;
                $this->longest_race_id = $this->selectedRunner->longest_race_id;
                $this->socks_size_id = $this->selectedRunner->socks_size_id;
                $this->shirt_size_id = $this->selectedRunner->shirt_size_id;
            }
            else
            {
                return redirect()->route('runners.list');
    ...
    public function edit()
    {
        ...
        $this->selectedRunner->name = $validatedData['name'];
        $this->selectedRunner->last_name = $validatedData['last_name'];
        $this->selectedRunner->email = $validatedData['email'];
        $this->selectedRunner->phone = $validatedData['phone'];
        $this->selectedRunner->sex = $validatedData['sex'];
        $this->selectedRunner->date_of_birth = $validatedData['date_of_birth'];
        $this->selectedRunner->work_position_id = $validatedData['work_position_id'] ?? null;
        $this->selectedRunner->work_sector_id = $validatedData['work_sector_id'] ?? null;
        $this->selectedRunner->week_running_id = $validatedData['week_running_id'] ?? null;
        $this->selectedRunner->longest_race_id = $validatedData['longest_race_id'] ?? null;
        $this->selectedRunner->socks_size_id = $validatedData['socks_size_id'] ?? null;
        $this->selectedRunner->shirt_size_id = $validatedData['shirt_size_id'] ?? null;
        $this->selectedRunner->save();

        session()->flash('message', 'Runner updated successfully.');
        return redirect()->route('runners.list');
    }
}; ?>
