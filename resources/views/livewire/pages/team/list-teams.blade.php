...
new class extends Component {
    ...
    protected $listeners = ['resetError', 'deleteSelectedTeam', 'deleteTeamConfirmed'];
    ...
    public function deleteSelectedTeam($captainId)
    {
        $this->captainId = $captainId;
        $this->dispatch('confirmDeletion', ['captainId' => $captainId]);
    }
    
    public function deleteTeamConfirmed()
    {
        if($this->captainId)
        {
            $captain = Captain::findOrFail($this->captainId);

            if($captain)
            {
                $captain->delete();
                $this->dispatch('pg:eventRefresh-teamsTable');
                session()->flash('message', 'Team deleted successfully.');                
            }            
        }

        $this->reset(['captainId', 'selectedCaptain']);
    }
}; ?>
...
<div>
    ...
    <div class="mt-6">
                ...
                @foreach($races as $race)
                    <div class="mx-1 h-full {{ $races->count() < 12 ? 'flex-1' : 'flex-none w-1/12' }}">
                        <button type="button"
                                ...
                                class="h-full w-full rounded shadow-md border p-3 text-white flex flex-col text-center justify-between"
                                :class="(@json($raceId) == @json($race->id))  ? '!border-light-green bg-gray-500' : 'border-gray-500 bg-gray-800'">
        ...
        <div class="mt-6 rounded bg-white p-4">
            <livewire:teams-table wire:key="teams-{{$year}}-{{$raceId}}" :race-id="$raceId" :year="$year"/>
        </div>
    </div>
    <script>
        document.addEventListener('livewire:init', function () {
            Livewire.on('confirmDeletion', (data) => {
                if (confirm('{{__('Are you sure you want to delete this team?')}}')) {
                    Livewire.dispatch('deleteTeamConfirmed');
                }
            });
        });
    </script>
