...
new class extends Component {
    ...
    protected $rules = [
        ...
        'file' => 'sometimes|nullable|file|max:16384|mimes:jpeg,jpg,png',
    ...
    public function mount($raceId)
    {
        if($raceId)
        {
            ...
            if($race)
            {
                ...
                $this->location = $this->selectedRace->location;
                $this->name = $this->selectedRace->name;
                $this->bill_prefix = $this->selectedRace->bill_prefix;
                $this->startplaces = $this->selectedRace->startplaces;
                $this->starting_date = Carbon::parse($this->selectedRace->starting_date)->format('Y-m-d');
                $this->application_start = Carbon::parse($this->selectedRace->application_start)->format('Y-m-d');
                $this->application_end = Carbon::parse($this->selectedRace->application_end)->format('Y-m-d');
                $this->logo = $this->selectedRace->logo;
                $this->web = $this->selectedRace->web;

                $inventory = $this->selectedRace->inventories()
                ...
                $this->inventory_name = $inventory->name;
                $this->inventory_description = $inventory->description;

                $inventoryIntervals = $inventory->inventoryIntervals;
                
                foreach($inventoryIntervals as $index => $ii)
                {
                    $this->intervals[$index]['id'] = $ii->id;
                    $this->intervals[$index]['name'] = $ii->name;
                    $this->intervals[$index]['price'] = $ii->price;
                    $this->intervals[$index]['start_date'] = Carbon::parse($ii->start_date)->format('Y-m-d');
                    $this->intervals[$index]['end_date'] = Carbon::parse($ii->end_date)->format('Y-m-d');
                }

                $inventoryExtra = $this->selectedRace->inventories()
                    ->whereHas('inventoryType', function ($query) {
                ...
                if($inventoryExtra)
                {
                    ...
                    $this->extra_name = $inventoryExtra->name;
                    $this->extra_abbreviation = $inventoryExtra->abbreviation;
                    $this->extra_description = $inventoryExtra->description;
                    $this->extra_price = $inventoryExtra->inventoryIntervals->first()->price;
                }      
                
                $promoCodes = $this->selectedRace->promoCodes;

                foreach($promoCodes as $index => $pc)
                {
                    $this->promoCodes[$index]['id'] = $pc->id;
                    $this->promoCodes[$index]['promo_code'] = $pc->promo_code;
                    $this->promoCodes[$index]['description'] = $pc->description;
                    $this->promoCodes[$index]['amount'] = $pc->amount;
                    $this->promoCodes[$index]['price'] = $pc->price;
                    $this->promoCodes[$index]['promo_type_id'] = $pc->promo_type_id;
                }
            }
            else
            {
    ...
    public function save()
    {
        ...
        $this->selectedRace->location = $validatedData['location'];
        $this->selectedRace->name = $validatedData['name'];
        $this->selectedRace->bill_prefix = $validatedData['bill_prefix'];
        $this->selectedRace->startplaces = $validatedData['startplaces'];
        $this->selectedRace->starting_date = $validatedData['starting_date'];
        $this->selectedRace->application_start = $validatedData['application_start'];
        $this->selectedRace->application_end = $validatedData['application_end'];
        $this->selectedRace->web = $validatedData['web'];

        if ($this->file) 
        {
            $path = $this->file->store('files', 'public');
            $this->selectedRace->logo = $path;
        }

        $this->selectedRace->save();

        $inventory = $this->selectedRace->inventories()
            ->whereHas('inventoryType', function ($q) {
                $q->where('inventory_type_name', 'Akontacija');
            })
            ->first();
        ...
        {
            $inventory->update([
                'name'        => $validatedData['inventory_name'] ?? $inventory->name,
                'description' => $validatedData['inventory_description'] ?? $inventory->description,
            ]);
        } 
        else 
        {
            $inventoryType = InventoryType::where('inventory_type_name', 'Akontacija')->first();
            $inventory = $this->selectedRace->inventories()->create([
        ...
        foreach ($this->intervals as $index => &$intervalData) 
        {
            $inventory = $this->selectedRace->inventories()
                ->whereHas('inventoryType', function ($q) {
                    $q->where('inventory_type_name', 'Akontacija');
            ...
            if (!is_null($intervalData['id'])) 
            {                
                $inventory->inventoryIntervals()
                ->where('id', $intervalData['id'])
                ->update([
            ...
            {
                $newInterval = $inventory->inventoryIntervals()
                    ->create([
                        'name'       => $intervalData['name'],
                        'price'      => $intervalData['price'],
                        'start_date' => $intervalData['start_date'],
                ...
                $intervalData['id'] = $newInterval->id;
            }
        }

        unset($intervalData);

        $newIntervalIds = collect($this->intervals)
            ->pluck('id')
            ->filter()
            ->toArray();

        $existingIntervalIds = $this->selectedRace->inventories()
            ->whereHas('inventoryType', function ($q) {
                $q->where('inventory_type_name', 'Akontacija');
            })
            ->first()
        ...
        $toDelete = array_diff($existingIntervalIds, $newIntervalIds);

        if (!empty($toDelete)) 
        {
            $this->selectedRace->inventories()
                ->whereHas('inventoryType', function ($q) {
                    $q->where('inventory_type_name', 'Akontacija');
                })
                ->first()
        ...
        $inventoryExtra = $this->selectedRace->inventories()
                ->whereHas('inventoryType', function ($q) {
                    $q->where('inventory_type_name', 'Extra');
                })
                ->first();
        ...
        {           
            if ($inventoryExtra) 
            {
                $inventoryExtra->update([
                    'name'        => $validatedData['extra_name'] ?? $inventoryExtra->name,
                    'abbreviation' => $validatedData['extra_abbreviation'] ?? $inventoryExtra->abbreviation,
                    'description' => $validatedData['extra_description'] ?? $inventoryExtra->description,
                ]);

                $inventoryExtraInterval = $inventoryExtra->inventoryIntervals->first();
                $inventoryExtraInterval->name = $inventoryExtra->name;
                $inventoryExtraInterval->price = $validatedData['extra_price'];
                $inventoryExtraInterval->start_date = $this->selectedRace->application_start;
                $inventoryExtraInterval->end_date = $this->selectedRace->application_end;
                $inventoryExtraInterval->save();
            } 
            else 
            {
                $inventoryTypeExtra = InventoryType::where('inventory_type_name', 'Extra')->first();

                $inventoryExtra = $this->selectedRace->inventories()->create([
                ...
                $inventoryExtra->inventoryIntervals()->create([
                    ...
                    'start_date' => $this->selectedRace->application_start,
                    'end_date' => $this->selectedRace->application_end,
                ]);
            }
        }
        else
        {
            if($inventoryExtra)
            {
                $inventoryExtra->delete();
            }
        }

        foreach ($this->promoCodes as $promoCodeData) 
        {
            if (!is_null($promoCodeData['id'])) 
            {
                $this->selectedRace->promoCodes()->where('id', $promoCodeData['id'])->update([
                    'promo_code' => $promoCodeData['promo_code'],
                    'description' => $promoCodeData['description'],
                    'promo_type_id' => $promoCodeData['promo_type_id'],
                    'amount' => $promoCodeData['amount'] ?? null,
                    'price' => $promoCodeData['price'] ?? null
                ]);
            } 
            else 
            {
                $newPromoCode = $this->selectedRace->promoCodes()->create([
                    'promo_code' => $promoCodeData['promo_code'],
                    'description' => $promoCodeData['description'],
                    'promo_type_id' => $promoCodeData['promo_type_id'],
                    'amount' => $promoCodeData['amount'] ?? null,
                ...
                $promoCodeData['id'] = $newPromoCode->id;
            }
        }

        unset($promoCodeData);

        $newPromoCodeIds = collect($this->promoCodes)
            ->pluck('id')
            ->filter()
            ->toArray();

        $existingPromoCodeIds = $this->selectedRace->promoCodes->pluck('id')->toArray();

        $toDeletePromoCodes = array_diff($existingPromoCodeIds, $newPromoCodeIds);

        if (!empty($toDeletePromoCodes)) 
        {
            $this->selectedRace->promoCodes()->whereIn('id', $toDeletePromoCodes)->delete();
        }

        session()->flash('message', 'Race updated successfully.');
        return redirect()->route('races.list');
    }
}; ?>
            ...
            <div class="grid grid-cols-3 justify-between gap-4">
                <div class="space-y-6 w-full">
                    ...
                    <div x-data="imageUpload()">
                        ...
                        @if($logo)
                            <div class="flex mt-2">
                                <img src="{{ asset('storage/' . $logo) }}" alt="{{__('Logo')}}" class="border border-black w-full object-cover rounded">
                            </div>
                        @endif
                        @if ($file)
                            <div class="mt-4">                         
