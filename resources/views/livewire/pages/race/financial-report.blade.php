...
<div>
    ...
    <div class="mt-6 rounded bg-white p-4" wire:loading.class="opacity-50">
        <div class="mb-4">
            <h3 class="font-bold text-lg">{{$selectedRace->name}}</h3>
            <p><b>{{__('Location')}}</b>: {{ $selectedRace->location }}</p>
            <p><b>{{__('Date')}}</b>: {{ Carbon::parse($selectedRace->starting_date)->format('d.m.Y.') }}</p>
        </div>
        <livewire:financial-report-table :race-id="$raceId"/>
    </div>
</div>
