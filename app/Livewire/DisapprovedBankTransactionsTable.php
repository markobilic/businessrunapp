...
final class DisapprovedBankTransactionsTable extends PowerGridComponent
{
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('duguje')
            ->add('potrazuje')
            ->add('potrazuje_in_eur', fn ($bankTransaction) => $bankTransaction->potrazuje ?? "0")
            ->add('potrazuje_copy')
            ->add('potrazuje_copy_in_eur', fn ($bankTransaction) => $bankTransaction->potrazuje_copy ?? "0")
            ...
            ->add('billing_data')
            ->add('debt')
            ->add('debt_in_eur', fn ($bankTransaction) => $bankTransaction->debt ?? "0")
            ->add('reservation_price')
            ->add('reservation_price_in_eur', fn ($bankTransaction) => $bankTransaction->reservation_price ?? "0")
            ->add('reservation_id')
            ->add('created_at')
            ->add('deleted_at');
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::add()
                ...
                ->sortable()
                ->searchable()
                ->editOnClick(hasPermission: true, dataField: 'potrazuje_copy_in_eur', fallback: '0', saveOnMouseOut: true),   
            Column::add()
                ->title(__('Total debt'))
